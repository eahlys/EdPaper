...
class DashController extends Controller
{
	...
	public function editAccount(Request $request){
		...
		if (Input::get('password') != "") {
			// dd($request);
			if (Input::get('password') != Input::get('passwordconfirm')) return Redirect::back()->withErrors(['Password confirmation does not match']);
			if (Hash::check(Input::get('currentpassword'), $user->password))
			{
				$user->password = Hash::make(Input::get('password'));
			}
			else return Redirect::back()->withErrors(['Current password incorrect']);
 		}
		$user->save();
